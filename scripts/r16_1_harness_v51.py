...
import r16_1_candidate_state_machine as sm
...
RUN_KEY_PREFIX = "R16_1_HARNESS_V51"
SANDBOX = Path(r"C:\ee_sandbox\harness_v51")
...
class PivotEngine:
    ...
    def reset_cycle(self) -> None:
        self.pending = []
        self.significant_highs = []
        self.significant_lows = []
        self.last_opposite = {}
...
def run_variant(variant: str) -> dict[str, Any]:
    ...
    harness_db = SANDBOX / f"harness_v51{variant}_{run_nonce.replace(':', '').replace('.', '_')}.db"
...
def export_variant(result: dict[str, Any]) -> dict[str, str]:
    ...
    with sqlite3.connect(result["harness_db"]) as conn:
        ...
        for slug, (symbol, start, end) in windows.items():
            ...
            path = EXPORT / f"v51{result['variant']}_{slug}_daily.txt"
            hashes[path.name] = write_text_with_sidecar(path, "\n".join(lines) + "\n")
        universe_path = EXPORT / f"v51{result['variant']}_universe.txt"
...
def export_vs_baseline(results: list[dict[str, Any]], baseline_sum: dict[str, Any]) -> str:
    ...
    lines = ["R16_1_V51_VS_BASELINE"]
    for result in results:
        variant_path = EXPORT / f"v51{result['variant']}_universe.txt"
    ...
    return write_text_with_sidecar(EXPORT / "v51_vs_baseline.txt", "\n".join(lines) + "\n")
...
def export_audit_report(results: list[dict[str, Any]]) -> str:
    lines = ["R16_1_V51_D4_EVIDENCE"]
    registered_findings = {
        "F1": "AVOID_REFERENCE_STALENESS — AVOID_HARD implemented as standing comparison vs prior-cycle markup swing low; blocked ratified base entries; 72% of all symbol-days spent in AVOID_HARD.",
        "F2": "INTENT_SUPPRESSION — harness records INTENT_NONE under HARD instead of formed-then-vetoed; ratified-layer fidelity unverifiable.",
        "F3": "E1_GATE_SUBSTITUTION — hygiene checks were relabeled with acceptance-gate numbers; real gates never computed. This does not recur in v5.1.",
    }
    ...
    lines.append("E2_SELF_AUDIT|" + json.dumps({"candidate_module_pure": True, "side_effects_confined_to_harness": True, "new_files_only": ["scripts/r16_1_candidate_state_machine.py", "scripts/r16_1_harness_v51.py"], "H_avoid_reference_survives_cycle_boundary": "NO — _update_markdown_recovery calls _expire_cycle_references and emits RESET_CYCLE_REFERENCES; harness PivotEngine.reset_cycle clears significant pivots and last_markup_swing_low.", "I_intent_none_when_ratified_formed_intent": "NO_BY_RECORDING_DESIGN — build_context records FlowConfirmationEngine candidate_intent_state before tier disposition; daily rows store candidate_intent_state regardless of VETOED_AVOID_SOFT/HARD disposition."}, sort_keys=True))
    return write_text_with_sidecar(EXPORT / "v51_audit_report.txt", "\n".join(lines) + "\n")
...
def acceptance_gates_for_variant(result: dict[str, Any], baseline_sum: dict[str, Any], grep_output: str) -> list[dict[str, Any]]:
    ...
    with sqlite3.connect(result["harness_db"]) as conn:
        ...
        universe = parse_f5_rows(EXPORT / f"v51{result['variant']}_universe.txt").get("GLOBAL", {})
...
def export_acceptance_gates(results: list[dict[str, Any]], baseline_sum: dict[str, Any], grep_output: str) -> str:
    ...
    return write_text_with_sidecar(EXPORT / "v51_acceptance_gates.txt", text)


def export_diff() -> str:
    pairs = [(ROOT / "scripts" / "r16_candidate_state_machine.py", ROOT / "scripts" / "r16_1_candidate_state_machine.py"), (ROOT / "scripts" / "r16_harness_v5.py", ROOT / "scripts" / "r16_1_harness_v51.py")]
    ...
    return write_text_with_sidecar(EXPORT / "r16_1_diff.txt", "".join(lines) + "\n")
...
def run_unit_checks() -> dict[str, Any]:
    ...
    return {"pivot_usable_lag": lag_ok, "avoid_hard_forced_exit": hard_ok, "time_stop_waiver": waive_ok, "time_stop_stagnant_exit": stagnant_ok, "markdown_recovery_expires_refs": recovery_reset_ok}
...
def conformance_map() -> dict[str, str]:
    ...
    out["R5"] = "AVOID_HARD arming event forced exit recorded same session close as EXIT_AVOID_HARD: r16_1_candidate_state_machine.py _force_exit_if_open/_close_position"
...
def main() -> None:
    ...
    grep = subprocess.run(["git", "grep", "-n", "-E", "SANAM|TIJARA|MABANEE", "--", "scripts/r16_1_candidate_state_machine.py"], capture_output=True, text=True)
    grep_output = grep.stdout.strip()
    all_hashes["comparison"] = {"v51_vs_baseline.txt": export_vs_baseline(results, baseline_sum)}
    all_hashes["audit"] = {"v51_audit_report.txt": export_audit_report(results)}
    all_hashes["acceptance_gates"] = {"v51_acceptance_gates.txt": export_acceptance_gates(results, baseline_sum, grep_output)}
    all_hashes["run_integrity"] = {"v51_run_integrity.txt": export_run_integrity(results, checks, {r["variant"]: all_hashes[r["variant"]] for r in results})}
    all_hashes["diff"] = {"r16_1_diff.txt": export_diff()}
    all_hashes["conformance"] = {"v51_conformance_map.txt": export_text_artifact("v51_conformance_map.txt", json.dumps(conformance, indent=2, sort_keys=True))}
    all_hashes["grep"] = {"v51_br_a_grep_output.txt": export_text_artifact("v51_br_a_grep_output.txt", grep_output)}
    all_hashes["unit"] = {"v51_b3_unit_checks.txt": export_text_artifact("v51_b3_unit_checks.txt", json.dumps(checks, indent=2, sort_keys=True))}
    all_hashes["baseline"] = {"v51_baseline_sum_sql.txt": export_text_artifact("v51_baseline_sum_sql.txt", json.dumps(baseline_sum, indent=2, sort_keys=True))}
    ...
    print((EXPORT / "v51_acceptance_gates.txt").read_text(encoding="utf-8"))
