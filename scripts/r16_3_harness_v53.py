...
RUN_KEY_PREFIX = "R16_3_HARNESS_V53"
SANDBOX = Path(r"C:\ee_sandbox\harness_v53")
EXPORT = SANDBOX / "export"
R3_INV_SOURCE = Path(r"C:\ee_sandbox\harness_v52\r3_inv_diff.txt")
...
def run_variant(variant: str) -> dict[str, Any]:
    ...
    harness_db = SANDBOX / f"harness_v53{variant}_{run_nonce.replace(':', '').replace('.', '_')}.db"
    ...
    with sqlite3.connect(str(harness_db)) as conn:
        ...
        for symbol, cfg in windows.items():
            ...
            pivot = PivotEngine(PIVOT_CONFIRMATION_LAG_SESSIONS, SIGNIFICANT_PIVOT_ATR_MULT)
...
def export_variant(result: dict[str, Any]) -> dict[str, str]:
    ...
    with sqlite3.connect(result["harness_db"]) as conn:
        ...
        for slug, (symbol, start, end) in windows.items():
            ...
            path = EXPORT / f"v53{result['variant']}_{slug}_daily.txt"
            hashes[path.name] = write_text_with_sidecar(path, "\n".join(lines) + "\n")
        universe_path = EXPORT / f"v53{result['variant']}_universe.txt"
...
def export_vs_baseline(results: list[dict[str, Any]], baseline_sum: dict[str, Any]) -> str:
    ...
    lines = ["R16_3_V53_VS_BASELINE"]
    for result in results:
        variant_path = EXPORT / f"v53{result['variant']}_universe.txt"
    ...
    return write_text_with_sidecar(EXPORT / "v53_vs_baseline.txt", "\n".join(lines) + "\n")


def export_efficiency_metric(results: list[dict[str, Any]], baseline_sum: dict[str, Any]) -> str:
    baseline = parse_f5_rows(BASELINE_EXPORT)
    base_global = baseline.get("GLOBAL") or baseline.get("GLOBAL_TOTALS") or {}
    baseline_sessions = float(base_global.get("total_open_sessions") or 0.0)
    baseline_efficiency = None if baseline_sessions <= 0.0 else float(baseline_sum["baseline_sum_pnl_pct"]) / baseline_sessions
    lines = ["R16_3_V53_REPORTED_METRIC_EFFICIENCY"]
    for result in results:
        candidate = parse_f5_rows(EXPORT / f"v53{result['variant']}_universe.txt").get("GLOBAL", {})
        candidate_sessions = float(candidate.get("total_open_sessions") or 0.0)
        candidate_sum = float(candidate.get("sum_pnl_pct") or 0.0)
        payload = {
            "variant": result["variant"],
            "baseline_sum_pnl_pct": baseline_sum["baseline_sum_pnl_pct"],
            "baseline_total_open_sessions": base_global.get("total_open_sessions"),
            "baseline_efficiency": baseline_efficiency,
            "candidate_sum_pnl_pct": candidate_sum,
            "candidate_total_open_sessions": candidate.get("total_open_sessions"),
            "candidate_efficiency": None if candidate_sessions <= 0.0 else candidate_sum / candidate_sessions,
        }
        lines.append("EFFICIENCY|" + json.dumps(payload, sort_keys=True))
    return write_text_with_sidecar(EXPORT / "v53_efficiency_metric.txt", "\n".join(lines) + "\n")
...
def export_audit_report(results: list[dict[str, Any]], checks: dict[str, Any]) -> str:
    ...
    registered_findings = {
        ...
        "R4": "SOFT_EVAL_STATE_GATING — S1-S3 were evaluated as an arming tier only in BASE_VALID/MARKUP_ACTIVE/AVOID_SOFT; v5.3 evaluates soft every session in every lifecycle state.",
        "R5": "CONFIRMED_INTENT_DISCARDED_IN_MARKUP — EXECUTE-class CONFIRMED_DIRECT from the ratified flow layer was not an entry path in MARKUP_ACTIVE; v5.3 adds M3 MARKUP_CONFIRMED_DIRECT.",
        "R6": "SAME_SESSION_REENTRY_FLIP — M1/M2/M3 re-entry could fire immediately after an exit; v5.3 requires an intervening close >= EMA10. Base-path entries remain exempt.",
    }
    ...
    lines.append("E2_SELF_AUDIT|" + json.dumps({"candidate_module_pure": True, "side_effects_confined_to_harness": True, "new_files_only": ["scripts/r16_3_candidate_state_machine.py", "scripts/r16_3_harness_v53.py"], "H_avoid_reference_survives_cycle_boundary": "NO — _update_markdown_recovery increments recovery_seq, calls _expire_cycle_references, and emits RESET_CYCLE_REFERENCES; harness PivotEngine.reset_cycle clears only last_markup_swing_low while preserving soft evidence.", "I_intent_none_when_ratified_formed_intent": "NO_BY_RECORDING_DESIGN — build_context records FlowConfirmationEngine candidate_intent_state before tier disposition; daily rows store candidate_intent_state regardless of VETOED_AVOID_SOFT/HARD disposition.", "J_adoption_promotion_seq_distinction": "BASE_VALID adoption is ratified-layer-authoritative and restamps originating_base_recovery_seq to current recovery_seq; BASE_RETIRED promotion remains gated by originating_base_recovery_seq == recovery_seq.", "C1_C2_C3_fixture_evidence": {"C1_soft_arms_from_neutral": bool(checks.get("C1_soft_arms_from_neutral")), "C2_m3_confirmed_direct": bool(checks.get("C2_m3_confirmed_direct")), "C3_reentry_blocked_until_ema10_recovery": bool(checks.get("C3_reentry_blocked_until_ema10_recovery"))}}, sort_keys=True))
    return write_text_with_sidecar(EXPORT / "v53_audit_report.txt", "\n".join(lines) + "\n")
...
def acceptance_gates_for_variant(result: dict[str, Any], baseline_sum: dict[str, Any], grep_output: str) -> list[dict[str, Any]]:
    ...
    with sqlite3.connect(result["harness_db"]) as conn:
        ...
        universe = parse_f5_rows(EXPORT / f"v53{result['variant']}_universe.txt").get("GLOBAL", {})
...
def export_acceptance_gates(results: list[dict[str, Any]], baseline_sum: dict[str, Any], grep_output: str) -> str:
    ...
    return write_text_with_sidecar(EXPORT / "v53_acceptance_gates.txt", text)


def export_diff() -> str:
    pairs = [(ROOT / "scripts" / "r16_2_candidate_state_machine.py", ROOT / "scripts" / "r16_3_candidate_state_machine.py"), (ROOT / "scripts" / "r16_2_harness_v52.py", ROOT / "scripts" / "r16_3_harness_v53.py")]
    lines: list[str] = ["R16.3_DIFFS"]
    ...
    return write_text_with_sidecar(EXPORT / "r16_3_diff.txt", "".join(lines) + "\n")
...
def run_unit_checks() -> dict[str, Any]:
    ...
    st8 = sm.initial_state("A")
    st8, actions8 = sm.step(st8, {**ctx, "close": 9.0, "ema30": 10.0, "ema30_slope_5s": -1.0, "usable_pivots": {"last_sig_high": {"price": 15.0, "obv": 9.0}, "prior_sig_high": {"price": 20.0, "obv": 10.0}}})
    c1_soft_arms_from_neutral = st8["lifecycle_state"] == "NEUTRAL" and bool(st8.get("avoid_soft_active")) and any(a.get("avoid_tier") == "AVOID_SOFT" for a in actions8)
    st9 = sm.initial_state("A")
    st9["lifecycle_state"] = "MARKUP_ACTIVE"
    st9, actions9 = sm.step(st9, {**ctx, "date": "20", "close": 12.0, "ema10": 10.0, "ema30": 9.0, "confirmation_state": "CONFIRMED", "candidate_intent_state": "EXECUTE_DIRECT", "usable_pivots": {}})
    c2_m3_confirmed_direct = any(a.get("entry_reason") == "MARKUP_CONFIRMED_DIRECT" for a in actions9)
    st10 = sm.initial_state("A")
    st10["lifecycle_state"] = "MARKUP_ACTIVE"
    st10["ema30_loss_streak"] = 1
    st10["position"] = {"position_id": "POS0003", "entry_date": "30", "entry_close": 10.0, "sessions_held": 1, "max_close": 10.0, "mfe": 0.0}
    reentry_ctx = {**ctx, "base_top_ref": None, "base_low_ref": None, "base_state": "NO_BASE", "base_valid": False, "base_mfe": 0.0, "confirmation_state": "CONFIRMED", "candidate_intent_state": "EXECUTE_DIRECT", "usable_pivots": {}}
    st10, actions10 = sm.step(st10, {**reentry_ctx, "date": "31", "close": 8.0, "ema10": 9.0, "ema30": 9.5})
    same_session_blocked = not any(a.get("type") == "OPEN_POSITION" for a in actions10)
    st10, actions11 = sm.step(st10, {**reentry_ctx, "date": "32", "close": 8.5, "ema10": 9.0, "ema30": 9.5})
    no_recovery_blocked = not any(a.get("type") == "OPEN_POSITION" for a in actions11)
    st10, actions12 = sm.step(st10, {**reentry_ctx, "date": "33", "close": 10.0, "ema10": 9.0, "ema30": 9.5})
    recovered_reentry = any(a.get("entry_reason") == "MARKUP_CONFIRMED_DIRECT" for a in actions12)
    c3_reentry_blocked_until_ema10_recovery = same_session_blocked and no_recovery_blocked and recovered_reentry
    return {"pivot_usable_lag": lag_ok, "avoid_hard_forced_exit": hard_ok, "time_stop_waiver": waive_ok, "time_stop_stagnant_exit": stagnant_ok, "markdown_recovery_expires_refs": recovery_reset_ok, "post_recovery_s1_persistent": post_recovery_s1_ok, "stale_seq_retirement_no_promote": stale_seq_retirement_no_promote, "K_adoption_restamps_then_promotes": k_adoption_restamps_then_promotes, "K_retired_without_current_adoption_neutral": k_no_adoption_retired_neutral, "C1_soft_arms_from_neutral": c1_soft_arms_from_neutral, "C2_m3_confirmed_direct": c2_m3_confirmed_direct, "C3_reentry_blocked_until_ema10_recovery": c3_reentry_blocked_until_ema10_recovery}
...
def conformance_map() -> dict[str, str]:
    ...
    out["R5"] = "AVOID_HARD arming event forced exit recorded same session close as EXIT_AVOID_HARD: r16_3_candidate_state_machine.py _force_exit_if_open/_close_position"
...
def main() -> None:
    ...
    grep = subprocess.run(["git", "grep", "-n", "-E", "SANAM|TIJARA|MABANEE", "--", "scripts/r16_3_candidate_state_machine.py"], capture_output=True, text=True)
    grep_output = grep.stdout.strip()
    all_hashes["comparison"] = {"v53_vs_baseline.txt": export_vs_baseline(results, baseline_sum)}
    all_hashes["efficiency"] = {"v53_efficiency_metric.txt": export_efficiency_metric(results, baseline_sum)}
    all_hashes["audit"] = {"v53_audit_report.txt": export_audit_report(results, checks)}
    all_hashes["acceptance_gates"] = {"v53_acceptance_gates.txt": export_acceptance_gates(results, baseline_sum, grep_output)}
    all_hashes["run_integrity"] = {"v53_run_integrity.txt": export_run_integrity(results, checks, {r["variant"]: all_hashes[r["variant"]] for r in results})}
    all_hashes["diff"] = {"r16_3_diff.txt": export_diff()}
    all_hashes["conformance"] = {"v53_conformance_map.txt": export_text_artifact("v53_conformance_map.txt", json.dumps(conformance, indent=2, sort_keys=True))}
    all_hashes["grep"] = {"v53_br_a_grep_output.txt": export_text_artifact("v53_br_a_grep_output.txt", grep_output)}
    all_hashes["unit"] = {"v53_b3_unit_checks.txt": export_text_artifact("v53_b3_unit_checks.txt", json.dumps(checks, indent=2, sort_keys=True))}
    all_hashes["baseline"] = {"v53_baseline_sum_sql.txt": export_text_artifact("v53_baseline_sum_sql.txt", json.dumps(baseline_sum, indent=2, sort_keys=True))}
    all_hashes["r3_inv_b"] = {"r3_inv_diff.txt": export_existing_artifact(R3_INV_SOURCE), "r3_inv_diff.txt.sha256": export_existing_artifact(R3_INV_SOURCE.with_suffix(R3_INV_SOURCE.suffix + ".sha256"))}
    ...
    print((EXPORT / "v53_acceptance_gates.txt").read_text(encoding="utf-8"))
