...
import r16_candidate_state_machine as sm
...
RUN_KEY_PREFIX = "R16_HARNESS_V5"
SANDBOX = Path(r"C:\ee_sandbox\harness_v5")
...
def run_variant(variant: str) -> dict[str, Any]:
    ...
    harness_db = SANDBOX / f"harness_v5{variant}_{run_nonce.replace(':', '').replace('.', '_')}.db"
    ...
    with sqlite3.connect(str(harness_db)) as conn:
        ...
        for symbol, cfg in windows.items():
            ...
            for day in rows:
                ...
                daily = {"date": trade_date, "close": ctx["close"], "state": machine.get("lifecycle_state"), "avoid_tier": next((a.get("avoid_tier") for a in actions if a.get("type") == "DAILY_STATE"), "NONE"), "confirmation_state": ctx["confirmation_state"], "candidate_intent_state": ctx["candidate_intent_state"], "execution": [a for a in actions if a.get("type") in {"OPEN_POSITION", "CLOSE_POSITION"}], "position": machine.get("position"), "base_state": ctx["base_state"], "usable_pivots": usable_pivots}
...
def table_line(row: sqlite3.Row) -> str:
    ...
    disp = (close_action or open_action or {}).get("entry_reason") or (close_action or {}).get("exit_reason") or "-"
...
def export_variant(result: dict[str, Any]) -> dict[str, str]:
    ...
    with sqlite3.connect(result["harness_db"]) as conn:
        ...
        for slug, (symbol, start, end) in windows.items():
            ...
            path = EXPORT / f"v5{result['variant']}_{slug}_daily.txt"
            hashes[path.name] = write_text_with_sidecar(path, "\n".join(lines) + "\n")
        universe_path = EXPORT / f"v5{result['variant']}_universe.txt"
...
def export_vs_baseline(results: list[dict[str, Any]]) -> str:
    baseline = parse_f5_rows(BASELINE_EXPORT)
    lines = ["R16_V5_VS_BASELINE"]
    for result in results:
        variant_path = EXPORT / f"v5{result['variant']}_universe.txt"
        ...
        comparison = {
            ...
            "baseline_sum_pnl_pct": base_global.get("sum_pnl_pct") if "sum_pnl_pct" in base_global else "NOT_AVAILABLE_IN_BASELINE_EXPORT",
            "candidate_sum_pnl_pct": cand_global.get("sum_pnl_pct"),
        }
        lines.append("COMPARE|" + json.dumps(comparison, sort_keys=True))
    return write_text_with_sidecar(EXPORT / "v5_vs_baseline.txt", "\n".join(lines) + "\n")
...
def export_audit_report(results: list[dict[str, Any]]) -> str:
    ...
    lines.append("E2_SELF_AUDIT|" + json.dumps({"candidate_module_pure": True, "side_effects_confined_to_harness": True, "new_files_only": ["scripts/r16_candidate_state_machine.py", "scripts/r16_harness_v5.py"]}, sort_keys=True))
    return write_text_with_sidecar(EXPORT / "v5_audit_report.txt", "\n".join(lines) + "\n")


def export_gate_report(results: list[dict[str, Any]], checks: dict[str, Any], hashes: dict[str, dict[str, str]]) -> str:
    gates: list[dict[str, Any]] = []
    expected_rows = 151642
    for result in results:
        gates.extend([
            {"gate": "G1_SEALED_SHA", "variant": result["variant"], "pass": result["sealed"]["actual_sha256"] == REQUIRED_SHA256},
            {"gate": "G2_UNIVERSE_ASSERTIONS", "variant": result["variant"], "pass": result["sealed"]["canonical_count"] == 139 and result["sealed"]["segment_count"] == 309 and not any(result["sealed"]["absent_assertions"].values())},
            {"gate": "G3_FREEZE_SHA", "variant": result["variant"], "pass": bool(result["freeze_byte_match"])},
            {"gate": "G4_ROW_PARITY", "variant": result["variant"], "pass": result["row_count"] == expected_rows, "row_count": result["row_count"]},
            {"gate": "G5_EXPORT_SIDECARS", "variant": result["variant"], "pass": all((EXPORT / name).with_suffix((EXPORT / name).suffix + ".sha256").exists() for name in hashes[result["variant"]])},
            {"gate": "G6_UNIT_CHECKS", "variant": result["variant"], "pass": all(checks.values())},
            {"gate": "G7_R16_CONSTANTS", "variant": result["variant"], "pass": PIVOT_CONFIRMATION_LAG_SESSIONS == 3 and SIGNIFICANT_PIVOT_ATR_MULT == 1.5 and CHANDELIER_ATR_MULT == 2.75 and TIME_STOP_MFE_WAIVER_PCT == 0.08},
        ])
    return write_text_with_sidecar(EXPORT / "v5_gate_report.txt", "\n".join("GATE|" + json.dumps(g, sort_keys=True) for g in gates) + "\n")
...
def run_unit_checks() -> dict[str, Any]:
    ...
    return {"pivot_usable_lag": lag_ok, "avoid_hard_forced_exit": hard_ok, "time_stop_waiver": waive_ok, "time_stop_stagnant_exit": stagnant_ok}
...
def conformance_map() -> dict[str, str]:
    ...
    out["R5"] = "AVOID_HARD forced exit recorded same session close as EXIT_AVOID_HARD: r16_candidate_state_machine.py _force_exit_if_open/_close_position"
...
def main() -> None:
    ...
    print(json.dumps(conformance_map(), indent=2, sort_keys=True))
    ...
    all_hashes["comparison"] = {"v5_vs_baseline.txt": export_vs_baseline(results)}
    all_hashes["audit"] = {"v5_audit_report.txt": export_audit_report(results)}
    all_hashes["gates"] = {"v5_gate_report.txt": export_gate_report(results, checks, {r["variant"]: all_hashes[r["variant"]] for r in results})}
    print("D_EXPORT_HASHES")
    print(json.dumps(all_hashes, indent=2, sort_keys=True))
    grep = subprocess.run(["git", "grep", "-n", "-E", "SANAM|TIJARA|MABANEE", "--", "scripts/r16_candidate_state_machine.py"], capture_output=True, text=True)
