...
C3_REENTRY_RECOVERY_SESSIONS = 1
...
def initial_state(variant: str) -> dict[str, Any]:
    return {
        ...
        "originating_base_recovery_seq": None,
        "recovery_seq": 0,
        "last_base_state": "NO_BASE",
        "avoid_soft_active": False,
        "soft_clear_streak": 0,
        "hard_base_top_breach_streak": 0,
        "markdown_recovery_streak": 0,
        "pullback_touch_age": None,
        "ema30_loss_streak": 0,
        "reentry_recovery_required": False,
        "reentry_recovery_seen": False,
        "reentry_recovery_count": 0,
        "last_exit_date": None,
    }
...
def step(machine_state: dict[str, Any], ctx: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if avoid_hard:
        ...
        state["lifecycle_state"] = "MARKDOWN"
    elif state["lifecycle_state"] == "MARKDOWN":
        if _update_markdown_recovery(state, ctx):
            actions.append({"type": "RESET_CYCLE_REFERENCES", "date": ctx.get("date"), "reason": "MARKDOWN_RECOVERY"})
    ...
    _update_reentry_recovery(state, ctx)
    ...
    actions.append(
        {
            ...
            "avoid_tier": "AVOID_HARD" if state.get("lifecycle_state") == "MARKDOWN" else _avoid_tier(avoid_soft, avoid_hard),
...
def _sync_base_lifecycle(state: dict[str, Any], ctx: dict[str, Any]) -> None:
    ...
    if state.get("lifecycle_state") == "MARKDOWN":
        return
    current_seq = int(state.get("recovery_seq") or 0)
    base_seq = ctx.get("base_recovery_seq")
    base_is_current_cycle = base_seq is None or int(base_seq) == current_seq
    ...
    if base_state == "BASE_RETIRED" and state.get("lifecycle_state") in {"NEUTRAL", "BASE_FORMING", "BASE_VALID", "RE_BASE"} and int(state.get("originating_base_recovery_seq") or -1) != current_seq:
        state["lifecycle_state"] = "NEUTRAL"
    elif base_state == "BASE_RETIRED" and state.get("lifecycle_state") == "BASE_VALID" and int(state.get("originating_base_recovery_seq") or -1) == current_seq and float(ctx.get("base_mfe") or 0.0) >= BASE_RETIREMENT_MFE_THRESHOLD:
...
def _capture_base_reference(state: dict[str, Any], ctx: dict[str, Any]) -> None:
    ...
    state["originating_base_recovery_seq"] = int(state.get("recovery_seq") or 0)
...
def _update_avoid_soft(state: dict[str, Any], conditions: dict[str, bool]) -> bool:
    active = sum(1 for value in conditions.values() if value) >= 2
    if active:
        state["avoid_soft_active"] = True
        state["soft_clear_streak"] = 0
        return True
    if bool(state.get("avoid_soft_active")):
        state["soft_clear_streak"] = int(state.get("soft_clear_streak") or 0) + 1
        if int(state["soft_clear_streak"]) >= SOFT_CLEAR_SESSIONS:
            state["avoid_soft_active"] = False
            state["soft_clear_streak"] = 0
            return False
        return True
    state["soft_clear_streak"] = 0
...
def _update_avoid_hard(state: dict[str, Any], ctx: dict[str, Any]) -> tuple[bool, str]:
    if state.get("lifecycle_state") not in {"MARKUP_ACTIVE", "BASE_VALID"}:
        state["hard_base_top_breach_streak"] = 0
        return False, "NONE"
    if state.get("lifecycle_state") == "BASE_VALID" and state.get("position") is None:
        state["hard_base_top_breach_streak"] = 0
        return False, "NONE"
    pivots = ctx.get("usable_pivots") or {}
...
def _update_markdown_recovery(state: dict[str, Any], ctx: dict[str, Any]) -> bool:
    ...
    if int(state.get("markdown_recovery_streak") or 0) >= 5:
        state["lifecycle_state"] = "NEUTRAL"
        state["markdown_recovery_streak"] = 0
        state["recovery_seq"] = int(state.get("recovery_seq") or 0) + 1
        _expire_cycle_references(state)
        return True
    return False


def _expire_cycle_references(state: dict[str, Any]) -> None:
    state["originating_base_top"] = None
    state["originating_base_low"] = None
    state["originating_base_id"] = None
    state["originating_base_recovery_seq"] = None
    state["hard_base_top_breach_streak"] = 0
...
def _entry_signal(state: dict[str, Any], ctx: dict[str, Any]) -> str | None:
    ...
    if state.get("lifecycle_state") != "MARKUP_ACTIVE":
        return None
    if str(ctx.get("confirmation_state") or "") == "CONFIRMED" and str(ctx.get("candidate_intent_state") or "").startswith("EXECUTE") and _reentry_allowed(state):
        return "MARKUP_CONFIRMED_DIRECT"
    pullback = _pullback_entry(state, ctx)
    if pullback and _reentry_allowed(state):
        return pullback
    if bool(ctx.get("flag_breakout")) and _reentry_allowed(state):
        return "MARKUP_FLAG_BREAKOUT"
    return None


def _reentry_allowed(state: dict[str, Any]) -> bool:
    if not bool(state.get("reentry_recovery_required")):
        return True
    if C3_REENTRY_RECOVERY_SESSIONS <= 0:
        return True
    return int(state.get("reentry_recovery_count") or 0) >= C3_REENTRY_RECOVERY_SESSIONS


def _update_reentry_recovery(state: dict[str, Any], ctx: dict[str, Any]) -> None:
    if state.get("position") is not None or not bool(state.get("reentry_recovery_required")):
        return
    if str(ctx.get("date")) == str(state.get("last_exit_date")):
        return
    if float(ctx.get("close") or 0.0) >= float(ctx.get("ema10") or 0.0):
        state["reentry_recovery_seen"] = True
        state["reentry_recovery_count"] = int(state.get("reentry_recovery_count") or 0) + 1
...
def _close_position(state: dict[str, Any], ctx: dict[str, Any], reason: str, detail: str | None = None) -> list[dict[str, Any]]:
    ...
    state["position"] = None
    state["ema30_loss_streak"] = 0
    state["reentry_recovery_required"] = True
    state["reentry_recovery_seen"] = False
    state["reentry_recovery_count"] = 0
    state["last_exit_date"] = ctx.get("date")
    return [action]
