...
HARNESS_DB = REVIEW / "r14b_module_a_harness_surface_v2.db"
...
def build_interface_conformance() -> dict[str, Any]:
    ...
    return {
        "version_id": "R14B_MODULE_A_INTERFACE_CONFORMANCE_V2",
        "pass": pass_all,
        "tables": per_table,
    }
...
def main() -> None:
    ...
    if HARNESS_DB.exists():
        raise FileExistsError(f"Harness DB already exists; refusing to mutate prior evidence surface: {HARNESS_DB}")
    ...
    impl_report_md = REVIEW / "r14b_module_a_implementation_report_v2.md"
    conformance_json = REVIEW / "r14b_module_a_interface_conformance_v2.json"
    evidence_json = REVIEW / "r14b_module_a_test_evidence_v2.json"
    ddl_sql = REVIEW / "r14b_module_a_schema_ddl_v2.sql"
    harness_log = REVIEW / "r14b_module_a_harness_output_v2.log"
    sidecar_chain = REVIEW / "r14b_module_a_daily_ledger_chain_v2.sha256"

    lines: list[str] = []
    lines.append("R14B_MODULE_A_HARNESS_V2_START")
    lines.append(f"HARNESS_DB {HARNESS_DB}")
    ...
    touched_files = [
        ...
        ROOT / "scripts" / "r14b_module_a_write_path_harness_v2.py",
    ]
    ...
    test_evidence = {
        "version_id": "R14B_MODULE_A_TEST_EVIDENCE_V2",
        "trade_date": trade_date,
    ...
    report = []
    report.append("# R14-B Module (a) Implementation Report v2")
    report.append("")
    report.append("Boundary: PredicateTelemetryLedger (storage + integrity only) on dedicated harness DB")
    report.append("")
    report.append("## File Hashes")
    report.append(json.dumps(touched_hashes, ensure_ascii=True, indent=2, sort_keys=True))
    report.append("")
    report.append("## Harness DB")
    report.append(str(HARNESS_DB))
    report.append("")
    ...
    print("R14B_MODULE_A_WRITE_PATH_HARNESS_V2_COMPLETE")
