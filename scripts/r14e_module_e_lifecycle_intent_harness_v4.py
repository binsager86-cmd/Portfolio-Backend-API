...
def record_daily_row(conn: sqlite3.Connection, symbol: str, trade_date: str, row: dict[str, Any]) -> None:
    conn.execute(
        ...
        VALUES (?, ?, ?, ?)
        """,
        ("R14E_MODULE_E_HARNESS_V4", symbol, trade_date, json.dumps(row, ensure_ascii=True, sort_keys=True)),
    )
    conn.commit()
...
def record_position_event(conn: sqlite3.Connection, symbol: str, trade_date: str, position_id: str, event_type: str, payload: dict[str, Any]) -> None:
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        ("R14E_MODULE_E_HARNESS_V4", symbol, trade_date, position_id, event_type, json.dumps(payload, ensure_ascii=True, sort_keys=True)),
    )
    conn.commit()
...
def main() -> None:
    ...
    out_interface = REVIEW / "r14e_module_e_interface_conformance_v4.json"
    out_evidence = REVIEW / "r14e_module_e_test_evidence_v4.json"
    out_report = REVIEW / "r14e_module_e_implementation_report_v4.md"
    out_sha = REVIEW / "r14e_module_e_artifacts_v4.sha256"
    harness_db = REVIEW / "r14e_module_e_harness_surface_v3.db"
    ...
    interface_payload = {
        "version_id": "R14E_MODULE_E_INTERFACE_CONFORMANCE_V4",
    ...
    evidence_payload = {
        "version_id": "R14E_MODULE_E_TEST_EVIDENCE_V4",
        "freeze_v2_attestation": attest,
        "run_nonce": RUN_NONCE,
        ...
        "avoid_plane_source_verbatim": "close < sma200 and sma200_slope < 0 and ema10 < ema30; clear via reclaim/20-session fallback",
        "mabanee_acceptance_check": "MABANEE must show avoid vetoes and zero EARLY_ACCUMULATION_ENTRY events during the decline window 2025-12-24 through 2026-02-01.",
    ...
    report_md = [
        ...
        "- MABANEE acceptance check: zero EARLY_ACCUMULATION_ENTRY events during 2025-12-24 through 2026-02-01 with avoid vetoes present.",
        "",
        "## Freeze Attestation",
    ...
    lines = [
        f"{sha256_file(out_interface)}  artifacts/preview1a_prestart/review_final/r14e_module_e_interface_conformance_v4.json",
        f"{sha256_file(out_evidence)}  artifacts/preview1a_prestart/review_final/r14e_module_e_test_evidence_v4.json",
        f"{sha256_file(out_report)}  artifacts/preview1a_prestart/review_final/r14e_module_e_implementation_report_v4.md",
    ]
    out_sha.write_text("\n".join(lines) + "\n", encoding="utf-8")

    print("R14E_MODULE_E_HARNESS_V4_COMPLETE")
