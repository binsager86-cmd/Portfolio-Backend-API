...
HARNESS_DB = REVIEW / "r14e_module_e_harness_surface_v5.db"
FREEZE_JSON = REVIEW / "r14b_parameter_freeze_v2.json"
FREEZE_SHA = REVIEW / "r14b_parameter_freeze_v2.sha256"
RUN_NONCE = "2026-07-18T08:45:41.5986908Z"

RUN_KEY = "R14E_MODULE_E_HARNESS_V5"
...
def owner_windows() -> dict[str, dict[str, Any]]:
    return {
        "MABANEE": {
            ...
            "owner_end": "2026-02-01",
            "replay_start": "2025-12-24",
            "replay_end": "2026-02-01",
            "source_artifact": "proposed_milestone_registry_v1.csv M7 peak 2025-12-24 through M9 late decline 2026-02-01; R12/F5 decline-window directive",
        },
        "SANAM": {
...
def main() -> None:
    ...
    out_interface = REVIEW / "r14e_module_e_interface_conformance_v5.json"
    out_evidence = REVIEW / "r14e_module_e_test_evidence_v5.json"
    out_report = REVIEW / "r14e_module_e_implementation_report_v5.md"
    out_sha = REVIEW / "r14e_module_e_artifacts_v5.sha256"
    harness_db = REVIEW / "r14e_module_e_harness_surface_v5.db"
    ...
    for symbol, cfg in windows.items():
        ...
        with sqlite3.connect(str(HARNESS_DB)) as ledger_conn:
            ...
            for idx, day in enumerate(rows):
                ...
                if execution_state in {"EXECUTE_EARLY_PILOT", "EXECUTE_CONFIRMED_ADD"}:
                    if position_state is None:
                        ...
                        position_state = {
                            ...
                            "pilot_fraction": float(route_out["execution_intent"].get("pilot_size_fraction") or 0.0),
                            "sessions_held": 0,
                            "rearm_count": 0,
                            "state": "OPEN",
                        }
                ...
                if position_state is not None:
                    ...
                    if position_state["sessions_held"] >= 60:
    ...
    acceptance_checks = {
        "MABANEE": {
            "check": "decline shows avoid_state=AVOID days and avoid-plane vetoes on any candidate/intent day within them and zero positions opened",
        ...
        "SANAM": {
            "check": "positions opened > 0 given 11 intents, or an explained veto for each",
    ...
    interface_payload = {
        "version_id": "R14E_MODULE_E_INTERFACE_CONFORMANCE_V5",
    ...
    evidence_payload = {
        "version_id": "R14E_MODULE_E_TEST_EVIDENCE_V5",
        "freeze_v2_attestation": attest,
        "run_nonce": RUN_NONCE,
        "harness_db": str(HARNESS_DB),
        "supersedes": {"v4": "SUPERSEDED_DEFECTIVE: avoid source empty/incorrect surface, SANAM window altered, ledger not accepted as connected"},
        ...
        "modules_f_g_authorization_note": "BLOCKED_PENDING_MODULE_E_REVIEW_PASS",
    }

    report_md = [
        "# R14-E Module (e) Harness v5",
        "",
        ...
        "- Supersedes: v4 SUPERSEDED_DEFECTIVE (avoid source empty/incorrect, SANAM window altered, ledger unfed/failed acceptance).",
        "- Modules (f)-(g): BLOCKED.",
        "",
        "## Freeze Attestation",
    ...
    lines = [
        f"{sha256_file(out_interface)}  artifacts/preview1a_prestart/review_final/r14e_module_e_interface_conformance_v5.json",
        f"{sha256_file(out_evidence)}  artifacts/preview1a_prestart/review_final/r14e_module_e_test_evidence_v5.json",
        f"{sha256_file(out_report)}  artifacts/preview1a_prestart/review_final/r14e_module_e_implementation_report_v5.md",
    ]
    out_sha.write_text("\n".join(lines) + "\n", encoding="utf-8")

    print("R14E_MODULE_E_HARNESS_V5_COMPLETE")
