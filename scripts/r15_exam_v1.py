...
RUN_KEY = "R15_EXAM_V1"
GRADER_VERSION = "R15_EXAM_GRADER_V1"
SET_A_WINDOWS = {
...
def load_v7_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("r14e_v7_source_for_r15", path)
...
def attest_freeze() -> dict[str, Any]:
    ...
    amend_actual = sha256_file(AMEND_JSON)
    return {
        ...
        "amendment_sha_sidecar": str(AMEND_SHA),
        "amendment_expected_sha256": sidecar_hash(AMEND_SHA),
        "amendment_actual_sha256": amend_actual,
        "amendment_byte_match": sidecar_hash(AMEND_SHA) == amend_actual,
...
def run_exam(attest: dict[str, Any]) -> dict[str, Any]:
    if not attest["freeze_byte_match"] or not attest["amendment_byte_match"]:
        raise RuntimeError("Freeze v2 or amendment 1 byte-match attestation failed")
    bind_harness_db()
    ...
    base = v7.AdaptiveBaseGeometry(v7.BaseNamedParameters(values={v7.BASE_MIN_SESSIONS: 10, v7.BASE_MAX_WIDTH_PCT: 0.24, v7.ATR_SQUEEZE_PCTILE: 0.95}))
    ...
    with sqlite3.connect(str(HARNESS_DB)) as conn:
        ...
        for symbol, cfg in SET_A_WINDOWS.items():
            ...
            for idx, day in enumerate(rows):
                ...
                if flow_out["candidate_intent"]["intent_state"] == "INTENT_FORMED" and route_out["execution_intent"]["execution_state"] == "NONE" and not route_out["veto_record"]["veto"] and router_current_state.get("active"):
    ...
    return {"daily_rows": daily_rows, "runtime_by_symbol": runtime_by_symbol, "position_ledger": position_ledger, "predictions": predictions, "grades": grades}
...
def decide_criteria(result: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    tijara_total = len(daily["TIJARA"])
    tijara_no_candidate = sum(1 for r in daily["TIJARA"] if r["candidate_intent"].get("intent_state") != "INTENT_FORMED")
    ...
    false_positive_cost = {"failed_pilot_count": len(cost_rows), "capital_days": sum(r["capital_days"] for r in cost_rows), "realized_drawdown_min": min((r["realized_drawdown"] for r in cost_rows), default=0.0), "rows": cost_rows}
    criteria = [
        ...
        {"criterion": "SANAM early-tier entry before 2025-05-08", "verdict": "PASS" if sanam_early_before else "FAIL", "expected": "FAIL under current detection timing (FLOW_CORE_LAG)", "actual_first_intent_date": sanam_first_intent, "required_before": "2025-05-08", "evidence_rows": sanam_early_before},
...
def write_report(attest: dict[str, Any], result: dict[str, Any], criteria: list[dict[str, Any]], false_positive_cost: dict[str, Any]) -> None:
    ...
    lines = ["# R15 Exam Report v1", "", f"RUN_NONCE: {RUN_NONCE}", f"RUN_KEY: {RUN_KEY}", "", "## Summary Verdict Block", "criterion|verdict", "---|---"]
    for row in criteria:
