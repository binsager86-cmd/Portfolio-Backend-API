...
HARNESS_DB = REVIEW / "r14e_module_e_harness_surface_v6.db"
FREEZE_JSON = REVIEW / "r14b_parameter_freeze_v2.json"
FREEZE_SHA = REVIEW / "r14b_parameter_freeze_v2.sha256"
RUN_NONCE = "2026-07-18T09:02:11.7669137Z"

RUN_KEY = "R14E_MODULE_E_HARNESS_V6"
...
def owner_windows() -> dict[str, dict[str, Any]]:
    return {
        "MABANEE": {
            "owner_start": "2025-12-24",
            "owner_end": "2026-04-30",
            "replay_start": "2025-12-24",
            "replay_end": "2026-04-30",
            "source_artifact": "owner directive for v6: start unchanged 2025-12-24, extended through 2026-04-30 to cover SMA200 crossing check",
        },
        "SANAM": {
...
def main() -> None:
    ...
    out_interface = REVIEW / "r14e_module_e_interface_conformance_v6.json"
    out_evidence = REVIEW / "r14e_module_e_test_evidence_v6.json"
    out_report = REVIEW / "r14e_module_e_implementation_report_v6.md"
    out_sha = REVIEW / "r14e_module_e_artifacts_v6.sha256"
    harness_db = REVIEW / "r14e_module_e_harness_surface_v6.db"
        ...
        with sqlite3.connect(str(HARNESS_DB)) as ledger_conn:
            ...
            for idx, day in enumerate(rows):
                ...
                if flow_out["candidate_intent"]["intent_state"] == "INTENT_FORMED":
                    ...
                    if route_out["execution_intent"]["execution_state"] == "NONE" and not route_out["veto_record"]["veto"]:
    ...
    r12_mabanee_window_dates = dates_in_intervals(r12_mabanee_intervals, [r["trade_date"] for r in avoid_sequences.get("MABANEE", [])])
    ...
    acceptance_checks = {
        "MABANEE": {
            "check": "post-crossing AVOID days present, avoid vetoes on candidate days within them, zero positions opened",
            "status": "PASS" if any(r["avoid_state"] == "AVOID" for r in avoid_sequences.get("MABANEE", [])) and summary_counts.get("MABANEE", {}).get("positions_opened") == 0 and (
                summary_counts.get("MABANEE", {}).get("early_intents_formed") == 0 or summary_counts.get("MABANEE", {}).get("vetoes_by_plane", {}).get("AVOID", 0) > 0
            ) else "FAIL",
        ...
        "SANAM": {
            "check": "positions opened > 0 from its intents with sessions_held progression",
    ...
    interface_payload = {
        "version_id": "R14E_MODULE_E_INTERFACE_CONFORMANCE_V6",
    ...
    evidence_payload = {
        "version_id": "R14E_MODULE_E_TEST_EVIDENCE_V6",
        "freeze_v2_attestation": attest,
        "run_nonce": RUN_NONCE,
        "harness_db": str(HARNESS_DB),
        "supersedes": {"v5": "SUPERSEDED: execution stage inert for confirmed first-touch intents; MABANEE window pre-crossing"},
        ...
        "module_code_remediation": "CONFIRMED_DIRECT_ENTRY implemented in lifecycle_intent_router.py per r14_design_spec_CONSOLIDATED_v2_2_addendum_1",
        ...
        "r12_mabanee_avoid_dates_in_v6_window": r12_mabanee_window_dates,
        "v6_sma200_mabanee_avoid_days": mabanee_avoid_days,
        "mabanee_r12_vs_v6_avoid_overlap": mabanee_overlap,
        "mabanee_r12_vs_v6_avoid_divergence": mabanee_divergence,
        ...
        "modules_f_g_authorization_note": "BLOCKED_PENDING_CLEAN_MODULE_E_V6",
    }

    report_md = [
        "# R14-E Module (e) Harness v6",
        "",
        ...
        "- Supersedes: v5 SUPERSEDED (execution stage inert; MABANEE avoid window pre-crossing).",
        "- Modules (f)-(g): BLOCKED.",
        "",
        "## Freeze Attestation",
        ...
        json.dumps({"r12_intervals": r12_mabanee_intervals, "r12_dates_in_v6_window": r12_mabanee_window_dates, "v6_sma200_avoid_days": mabanee_avoid_days, "overlap": mabanee_overlap, "divergence": mabanee_divergence}, ensure_ascii=True, indent=2, sort_keys=True),
        "",
    ...
    lines = [
        f"{sha256_file(out_interface)}  artifacts/preview1a_prestart/review_final/r14e_module_e_interface_conformance_v6.json",
        f"{sha256_file(out_evidence)}  artifacts/preview1a_prestart/review_final/r14e_module_e_test_evidence_v6.json",
        f"{sha256_file(out_report)}  artifacts/preview1a_prestart/review_final/r14e_module_e_implementation_report_v6.md",
    ]
    out_sha.write_text("\n".join(lines) + "\n", encoding="utf-8")

    print("R14E_MODULE_E_HARNESS_V6_COMPLETE")
