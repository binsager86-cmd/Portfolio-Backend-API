...
def _compute_data_quality_score(
    ...
    *,
    ...
    if latest.get("corporate_actions") is None:
        ...
        score -= 5.0
    if latest.get("corporate_action_integrity") != "VERIFIED":
        reasons.append("corporate_action_integrity_unverified")
        score -= 15.0
...
async def generate_kuwait_signal(
    ...
    # Daily OHLC range is not a bid/ask spread. Without quoted bid/ask data,
    # keep spread unavailable and exclude it from execution costs.
    ...
    spread_cost_pct = max(0.0, spread_pct or 0.0)
    ...
    entry_cost_fils = entry_plan * (TC_COMMISSION + slippage_rate + spread_cost_pct / 200.0)
    stop_cost_fils = stop_plan * (TC_COMMISSION + slippage_rate + spread_cost_pct / 200.0)
    gap_samples = [
    ...
    for target_key, target_price in served_targets.items():
        if target_price is None or risk_fils <= 0:
            continue
        target_cost_fils = float(target_price) * (TC_COMMISSION + slippage_rate + spread_cost_pct / 200.0)
    ...
    levels["net_rr"] = round(
        (abs(float(levels.get("tp1") or 0.0) - entry_plan) - entry_cost_fils - (float(levels.get("tp1") or 0.0) * (TC_COMMISSION + slippage_rate + spread_cost_pct / 200.0))) / net_risk_fils,
        3,
    ...
    # Circuit handling may change the served TP1. Refresh every dependent
    # plan metric before risk, probability, sizing, and gates consume it.
    entry_plan = float(levels.get("entry_mid") or 0.0)
    stop_plan = float(levels.get("stop_loss") or 0.0)
    risk_fils = abs(entry_plan - stop_plan)
    net_risk_fils = risk_fils + entry_cost_fils + stop_cost_fils + gap_risk_fils
    rr = round(abs(float(levels.get("tp1") or 0.0) - entry_plan) / risk_fils, 2) if levels.get("tp1") is not None and risk_fils > 0 else 0.0
    levels["risk_per_share"] = round(risk_fils, 1) if risk_fils > 0 else None
    levels["risk_reward_ratio"] = rr
    levels["gross_rr"] = round(rr, 3)
    levels["target_metrics"] = {}
    for target_key in ("tp1", "tp2", "tp3"):
        target_price = levels.get(target_key)
        if target_price is None or risk_fils <= 0:
            continue
        target_cost = float(target_price) * (TC_COMMISSION + slippage_rate + spread_cost_pct / 200.0)
        gross_reward = abs(float(target_price) - entry_plan)
        levels["target_metrics"][target_key] = {
            "price": float(target_price),
            "methods": (levels.get("tp_methods") or {}).get(target_key),
            "confidence": (levels.get("tp_methods") or {}).get(f"{target_key}_confluence"),
            "distance_fils": round(gross_reward, 2),
            "gross_rr": round(gross_reward / risk_fils, 3),
            "net_rr": round((gross_reward - entry_cost_fils - target_cost) / net_risk_fils, 3) if net_risk_fils > 0 else None,
        }
    if levels.get("tp1") is not None and net_risk_fils > 0:
        tp1_cost = float(levels["tp1"]) * (TC_COMMISSION + slippage_rate + spread_cost_pct / 200.0)
        levels["net_rr"] = round((abs(float(levels["tp1"]) - entry_plan) - entry_cost_fils - tp1_cost) / net_risk_fils, 3)
    else:
        levels["net_rr"] = None
    ...
    if regime_upper in {"BULLISH_EXPANSION", "BULLISH", "BULL"}:
        regime_direction = 0.6
    elif regime_upper in {"BEARISH_CONTRACTION", "BEARISH", "BEAR"}:
        regime_direction = -0.6
    ...
    four_scores = compute_all_four_scores(
        ...
        spread_pct=float(spread_cost_pct),
    ...
    component_scores = {
        "trend": _component_contract(
            ...
            available=component_available["trend"],
        ...
        "momentum": _component_contract(
            ...
            available=component_available["momentum"],
        ...
        "volume_flow": _component_contract(
            ...
            available=component_available["volume_flow"],
        ...
        "support_resistance": _component_contract(
            ...
            confidence=0.7,
            available=component_available["support_resistance"],
            details={
        ...
        "risk_reward": _component_contract(
            ...
            available=component_available["risk_reward"],
        ...
        "regime": _component_contract(
            ...
            available=bool(regime_result.get("current_regime")),
    ...
    confluence = {
        ...
        "spread": {"available": spread_pct is not None, "value_pct": spread_pct, "source": "bid_ask_quote" if spread_pct is not None else None},
